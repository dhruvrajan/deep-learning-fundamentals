...
def create_trainer(model: nn.Module, optimizer: Adam, loss_function):
...
def create_evaluator(model: nn.Module, metrics: dict):
...
def train_simple_ffnn(
    ...
    val_loader = DataLoader(test_tensors, batch_size=args.batch_size, shuffle=True)
    ...
    trainer = create_trainer(model, optimizer, loss_function)
